...
void File_Jpeg::SIZ()
{
    ...
    FILLING_BEGIN_PRECISE();
        ...
        Fill(StreamKind, 0, StreamKind==Stream_Image?Image_Width:Video_Width, Xsiz);
        Fill(StreamKind, 0, StreamKind==Stream_Image?Image_Height:Video_Height, Ysiz);
        Accept("JPEG 2000");
    FILLING_END();
}
...
void File_Jpeg::SOF_()
{
    ...
    FILLING_BEGIN_PRECISE();
        if (!IsAccepted)
        {
            ...
            if (Count_Get(StreamKind)==0)
                Stream_Prepare(StreamKind);
            Fill(StreamKind, 0, StreamKind==Stream_Image?Image_Format:Video_Format, StreamKind==Stream_Image?"JPEG":"M-JPEG");
            Fill(StreamKind, 0, StreamKind==Stream_Image?Image_Codec:Video_Codec, StreamKind==Stream_Image?"JPEG":"M-JPEG");
            if (StreamKind==Stream_Image)
                Fill(Stream_Image, 0, Image_Codec_String, "JPEG", Unlimited, true, true); //To Avoid automatic filling
            Fill(StreamKind, 0, StreamKind==Stream_Image?Image_Resolution:Video_Resolution, Resolution*3);
            Fill(StreamKind, 0, StreamKind==Stream_Image?Image_Height:Video_Height, Height*Height_Multiplier);
            Fill(StreamKind, 0, StreamKind==Stream_Image?Image_Width:Video_Width, Width);
            Accept("JPEG");
        }
    FILLING_END();
}
