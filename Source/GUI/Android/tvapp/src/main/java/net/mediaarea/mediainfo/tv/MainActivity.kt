...
private fun MediaInfoApp(drawerState: DrawerState = rememberDrawerState(initialValue = DrawerValue.Open)) {
    Surface(
        ...
        val items = listOf(
            Screen.Browse, Screen.Settings, Screen.About
        )

        NavigationDrawer(
            drawerState = drawerState, drawerContent = { drawerValue ->
                Column(
                    modifier = Modifier
                        .background(
...
private sealed class Screen(
    val route: String, @get:StringRes val resourceId: Int, val iconRes: Int
) {
    object Browse : Screen("browse", R.string.browse, R.drawable.browse_24px)
    object Settings : Screen("settings", R.string.settings, R.drawable.settings_24px)
    object About : Screen("about", R.string.about, R.drawable.info_24px)
}
